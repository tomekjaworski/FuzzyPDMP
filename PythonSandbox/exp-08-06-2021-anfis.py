...
class FuzzyOperation(object):
    ...
    def Centroid(self, domain: np.ndarray, aggregatedVariable: np.ndarray) -> Optional[float]:
        if not isinstance(domain, np.ndarray):
            raise TypeError("domain")
        ...
        aggregatedVariable = aggregatedVariable * 0

        # Czy jest coś do defuzyfikacji?
        if not np.any(aggregatedVariable):
            return None
...
class Model_Przekaz_tresc(object):
    ...
    def InitVariables(self, parameters: np.array):

        self.parameters = np.array(parameters) # kopiuj parametry

        # Wejście: censydiam1
        self.memb_Censydiam1_Zabawa = fuzz.trapmf(self.x_Censydiam1, parameters[0,:])
        self.memb_Censydiam1_Kontrola = fuzz.trapmf(self.x_Censydiam1, parameters[1,:])

        # Wejście: censydiam2
        self.memb_Censydiam2_Witalnosc = fuzz.trapmf(self.x_Censydiam2, parameters[2,:])
        self.memb_Censydiam2_Wyciszenie = fuzz.trapmf(self.x_Censydiam2, parameters[3,:])

        # Wejście: censydiam3
        self.memb_Censydiam3_Status = fuzz.trapmf(self.x_Censydiam3, parameters[4,:])
        self.memb_Censydiam3_Przynaleznosc = fuzz.trapmf(self.x_Censydiam3, parameters[5,:])

        # Wejście: censydiam4
        self.memb_Censydiam4_Dzielenie = fuzz.trapmf(self.x_Censydiam4, parameters[6,:])
        self.memb_Censydiam4_Wyroznianie = fuzz.trapmf(self.x_Censydiam4, parameters[7,:])

        # Wyjście: tresc_przekazu
        self.memb_Tresc_przekazu_Kontrola = fuzz.trapmf(self.x_Tresc_przekazu, parameters[8,:])
        self.memb_Tresc_przekazu_Zabawa = fuzz.trapmf(self.x_Tresc_przekazu, parameters[9,:])

    def GetParameters(self, makeCopy: bool = True) -> np.array:
        if makeCopy:
...
if __name__ == "__main__":
    ...
    m0_params = np.array([
        # Wejście: Censydiam1
        [0.0000, 0.0000, 1.0000, 9.0000],  # Censydiam1_Zabawa
        [1.0000, 9.0000, 10.0000, 10.0000],  # Censydiam1_Kontrola

        # Wejście: censydiam2
        [0.0000, 0.0000, 1.0000, 9.0000],  # Censydiam2_Witalnosc
        [1.0000, 9.0000, 10.0000, 10.0000],  # Censydiam2_Wyciszenie

        # Wejście: censydiam3
        [0.0000, 0.0000, 1.0000, 9.0000],  # Censydiam3_Status
        [1.0000, 9.0000, 10.0000, 10.0000],  # Censydiam3_Przynaleznosc

        # Wejście: censydiam4
        [0.0000, 0.0000, 1.0000, 9.0000],  # Censydiam4_Dzielenie
        [1.0000, 9.0000, 10.0000, 10.0000],  # Censydiam4_Wyroznianie

        # Wyjście: tresc_przekazu
        [0.0000, 0.0000, 1.0000, 9.0000],  # Tresc_przekazu_Kontrola
        [1.0000, 9.0000, 10.0000, 10.0000],  # Tresc_przekazu_Zabawa
    ])

    m0.InitVariables(m0_params)
    m0.ShowAllVariables()
    m0_result = m0.Execute(np.random.choice(m0.x_Censydiam1), np.random.choice(m0.x_Censydiam2), np.random.choice(m0.x_Censydiam3),
                           np.random.choice(m0.x_Censydiam4))
    m0.ShowActivations()
